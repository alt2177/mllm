...
from transformers import DataCollatorWithPadding, BertForSequenceClassification, TrainingArguments, Trainer, BertConfig  
...
def main():
    ...
    username = "mllm-dev"
    output_repo = "yelp_finetuned_phi-1.5_6gpu"

    # load and tokenize data
    dataset = load_dataset("yelp_review_full")
    tokenizer = AutoTokenizer.from_pretrained("microsoft/phi-1_5", use_auth_token = access_token, trust_remote_code=True)
    tokenizer.pad_token = tokenizer.eos_token

    tokenized_yelp = dataset.map(lambda examples:tokenizer(examples["text"], truncation=True,max_length=2048),batched=True)
    
    # pad tokens
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer) 

    # create model
    config = BertConfig.from_pretrained("microsoft/phi-1_5", trust_remote_code=True)
    ...
    output_dir = "yelp_finetune_phi1.5_test_gpu6_2"
    # training loop
...
#	gradient_accumulation_steps=4,   #want to explore what this is
    ...
    torch.save(probs,"test_probs_yelp_phi1.5.pt")
   # trainer.evaluate()
