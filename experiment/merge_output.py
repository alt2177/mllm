...
for i in range(num_experiments):
    filename = f"final_layer_predictions/gpt2_f_experiment_{i}_train.pt"
...
for i in range(num_experiments):
    filename = f"final_layer_predictions/gpt2_f_experiment_{i}_test.pt"
...
eval_dataset = dataset['test'].map(lambda example, idx: {'index': idx, **example}, with_indices=True).shuffle(42)
...
model = NeuralNetwork(input_dim=5 * num_experiments,hidden_dim=20,output_dim=5).to(device)
...
test_prob_reshaped = test_prob.reshape(test_prob.shape[0], 10, 5)

# Split test_prob_reshaped along the second axis to get 5 tensors
tensors_list = np.split(test_prob_reshaped, 10, axis=1)
...
# Reshape test_prob to have 5 sets of 10 columns
validation_prob_reshaped = validation_prob.reshape(validation_prob.shape[0], 10, 5)

# Split test_prob_reshaped along the second axis to get 5 tensors
tensors_list = np.split(validation_prob_reshaped, 10, axis=1)
