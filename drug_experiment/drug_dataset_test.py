...
def main():
    ...
    for m in range(5):
        ...
        output_repo = f"gpt2_f_experiment_{m}_drug_data_new_run"

        # Select 5% of the train data for finetuning a specific model
        num_of_samples = int(len(tokenized_drug_train) * 0.2)
        end_index = start_index + num_of_samples
        
        train_dataset = tokenized_drug_train.shuffle(seed=42).select(range(start_index, end_index))
        ...
        output_dir = "tam_test_out_drug_data_new_run"
        # training loop
        ...
        f.write(f"Fine tune model {m} all train data result : {predictions.metrics}\n")
